...
Future<String?> generateExcelReport({
  ...
  required List<String> columnsToTotal,
  ...
  try {
    ...
    dateRangeCell.value = TextCellValue("Period: $startDateStr to $endDateStr");
    ...
    for (final column in columnsToTotal) {
      totals[column] = 0;
    }
    ...
    if (columnsToTotal.isNotEmpty) {
      ...
      for (final header in headers) {
        ...
        if (columnsToTotal.contains(header)) {
          cell.value = DoubleCellValue(totals[header]!.toDouble());
          cell.cellStyle = CellStyle(
            bold: true,
          );
        } else if (colIndex == 0) {
